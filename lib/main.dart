...
const Color cOrangeLight = Color(0xFFFFF7ED);
...
Branch? nearestBranchInRange(double lat, double lng) {
  Branch? nearest;
  double minDist = double.infinity;
  for (final b in kBranches) {
    final d = haversineDistance(lat, lng, b.lat, b.lng);
    if (d <= b.radiusMeters && d < minDist) {
      minDist = d;
      nearest = b;
    }
  }
...
class AppHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: const BoxDecoration(
      gradient: LinearGradient(colors: [cNavyMid, cNavy],
...
class RoleBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final styles = {'Admin':[cAmberLight,cAmberText],'Manager':[cVioletLight,cVioletText],'Staff':[cGreenLight,cGreenText]};
...
class ErrorBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Icon(Icons.location_off_rounded, color: cRed, size: 18),
    ...
    child: raisedCard(radius: 10, child: Padding(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(u.name, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: cText),
    ...
    Expanded(child: _bgGradient(child: SingleChildScrollView(padding: const EdgeInsets.all(24),
      child: Column(children: [
        ...
        Container(width: 76, height: 76,
          decoration: BoxDecoration(
            gradient: const LinearGradient(colors: [cNavyMid, cNavyLight], begin: Alignment.topLeft, end: Alignment.bottomRight),
            borderRadius: BorderRadius.circular(20),
  ...
  Widget build(BuildContext context) => GestureDetector(onTap: onTap,
    child: Container(width: double.infinity, padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        boxShadow: [
          BoxShadow(color: gradient.last.withOpacity(0.45), blurRadius: 12, offset: const Offset(0,5)),
...
class _ClockInScreenState extends State<ClockInScreen> {
  ...
  /// Simulates requesting device GPS.
  /// In the real app replace this body with geolocator package calls:
  ...
  Future<void> _requestGps() async {
    ...
    final Position p = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);
    final simLat = p.latitude;
    final simLng = p.longitude;
    ...
    if (branch != null) {
      ...
      // Calculate distances to both branches for the error message
      final distAirport = haversineDistance(simLat, simLng,
          kBranches[0].lat, kBranches[0].lng);
      final distCity = haversineDistance(simLat, simLng,
          kBranches[1].lat, kBranches[1].lng);
      final closerName = distAirport < distCity
          ? kBranches[0].name : kBranches[1].name;
  ...
  void _confirmClockIn() {
    ...
    Future.delayed(const Duration(milliseconds: 800), () => widget.onClockedIn(record));
  }
    ...
    Expanded(child: _bgGradient(child: SingleChildScrollView(padding: const EdgeInsets.all(16),
      child: Column(children: [
        ...
        raisedCard(radius: 16, child: Padding(padding: const EdgeInsets.all(20),
          child: Column(children: [
            Text(_timeStr(), style: const TextStyle(fontSize: 42, fontWeight: FontWeight.w900,
            ...
            // ── Selfie area (only shown after GPS verified) ──
            if (gpsState == GpsState.granted) ...[
              GestureDetector(onTap: () => setState(() => selfieOk = !selfieOk),
                child: Container(width: double.infinity, height: 86,
                  decoration: BoxDecoration(
                    color: selfieOk ? cGreenLight : cSlate,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: selfieOk ? cGreenBorder : cBorder, width: 1.5)),
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(selfieOk ? Icons.check_circle_rounded : Icons.camera_alt_rounded,
                        color: selfieOk ? cGreen : cMuted, size: 26),
                    const SizedBox(height: 5),
                    Text(selfieOk ? 'Selfie captured ✓' : 'Take selfie to clock in',
                        style: TextStyle(color: selfieOk ? cGreenText : cMuted, fontSize: 12,
        ...
        // ── Branches reference card ──
        raisedCard(radius: 14, child: Padding(padding: const EdgeInsets.all(16),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Row(children: [
              Icon(Icons.store_rounded, color: cNavyMid, size: 16),
              SizedBox(width: 8),
              Text('Authorised clock-in locations',
                  style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: cNavyMid))]),
            const SizedBox(height: 12),
            ...kBranches.map((b) => Padding(
              padding: const EdgeInsets.only(bottom: 10),
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Container(width: 8, height: 8, margin: const EdgeInsets.only(top: 4),
                    decoration: BoxDecoration(color: cGreen, borderRadius: BorderRadius.circular(4))),
                const SizedBox(width: 10),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(b.name, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 12, color: cText)),
                  Text(b.address, style: const TextStyle(fontSize: 11, color: cMuted, height: 1.4))]))])))]))),
...
class _GpsPanel extends StatelessWidget {
  ...
  const _GpsPanel({required this.gpsState, required this.detectedBranch,
      required this.gpsError, required this.capturedLat, required this.capturedLng,
      required this.gpsTimestamp, required this.onVerify});
  ...
  Widget build(BuildContext context) {
    switch (gpsState) {
      ...
      // ── IDLE: tap to verify ──
      case GpsState.idle:
        return GestureDetector(onTap: onVerify,
          child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 14),
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(width: 8),
              Text('Tap to verify location', style: TextStyle(color: cMuted, fontSize: 13, fontWeight: FontWeight.w600))])));
      ...
      case GpsState.loading:
        return Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 14),
          ...
          child: const Column(children: [
            ...
            SizedBox(height: 8),
            Text('Acquiring GPS signal…', style: TextStyle(color: cMuted, fontSize: 12))]));

      // ── GRANTED (in range) ──
      case GpsState.granted:
        return Container(width: double.infinity, padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(color: cGreenLight, borderRadius: BorderRadius.circular(10),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 6),
            Text(detectedBranch!.address,
                style: const TextStyle(color: cGreenText, fontSize: 11, height: 1.3)),
            const SizedBox(height: 8),
            // Coords + timestamp
            Container(padding: const EdgeInsets.all(8),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 3),
                Row(children: [
                  const Icon(Icons.access_time_rounded, color: cGreen, size: 12),
                  const SizedBox(width: 4),
                  Text('Captured at ${_fmtTs(gpsTimestamp)}',
          ...
          Container(width: double.infinity, padding: const EdgeInsets.all(14),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Row(children: [
                ...
                SizedBox(width: 8),
                Text('Location Out of Range', style: TextStyle(color: cOrangeDark,
              ...
              GestureDetector(onTap: onVerify,
                child: Container(padding: const EdgeInsets.symmetric(vertical: 9),
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(colors: [cOrange, Color(0xFFEA580C)]),
                    borderRadius: BorderRadius.circular(8),
                    boxShadow: [BoxShadow(color: cOrange.withOpacity(0.35), blurRadius: 6, offset: const Offset(0,2))]),
                  child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(Icons.refresh_rounded, color: Colors.white, size: 16),
                    SizedBox(width: 6),
                    Text('Retry GPS Check', style: TextStyle(color: Colors.white,
          ...
          if (capturedLat != null) ...[
            ...
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7),
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 6),
                Text('Last GPS: ${capturedLat!.toStringAsFixed(5)}, ${capturedLng!.toStringAsFixed(5)}',
      ...
      case GpsState.denied:
        return Container(padding: const EdgeInsets.all(12),
          ...
          child: Column(children: [
            ...
            const SizedBox(height: 6),
            const Text('Please enable location access in your device Settings to clock in.',
...
class _ClockInButton extends StatelessWidget {
  final GpsState gpsState; final bool selfieOk, clockedIn;
  final VoidCallback onTap;
  const _ClockInButton({required this.gpsState, required this.selfieOk,
      required this.clockedIn, required this.onTap});

  bool get _enabled => gpsState == GpsState.granted && selfieOk && !clockedIn;

  @override
  Widget build(BuildContext context) {
    if (clockedIn) return Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 14),
      decoration: BoxDecoration(color: cGreenLight, borderRadius: BorderRadius.circular(12),
          border: Border.all(color: cGreenBorder)),
      child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.check_circle_rounded, color: cGreen, size: 20),
        SizedBox(width: 8),
        Text('Clocked In Successfully!', style: TextStyle(color: cGreenText, fontWeight: FontWeight.w800, fontSize: 15))]));
      ...
      child: GestureDetector(onTap: _enabled ? onTap : null,
        child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 15),
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(12),
            boxShadow: _enabled ? [
              BoxShadow(color: cGreen.withOpacity(0.45), blurRadius: 10, offset: const Offset(0,4)),
              const BoxShadow(color: Color(0xFF064E3B), blurRadius: 0, offset: Offset(0,3))] : []),
          child: Text(
            gpsState != GpsState.granted
                ? 'Verify Location First'
                : !selfieOk ? 'Take Selfie to Enable' : 'Clock In',
...
class _ClockOutScreenState extends State<ClockOutScreen> {
  ...
  String _shiftTotal() {
    if (widget.clockInRecord == null) return '0.0 hrs';
        ...
        if (confirmed) ...[
          ...
          Container(padding: const EdgeInsets.all(24),
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 6),
              Text('Shift total: ${_shiftTotal()}', style: const TextStyle(color: cGreenText, fontSize: 14)),
              const SizedBox(height: 16),
          ...
          raisedCard(radius: 16, child: Padding(padding: const EdgeInsets.all(18),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const Divider(height: 20, color: cBorder),
              _SummaryRow(label: 'Shift total', value: _shiftTotal(), bold: true, valueColor: cBlue)]))),
          const SizedBox(height: 12),
          // Branch info from clock-in record
          raisedCard(radius: 12, child: Padding(padding: const EdgeInsets.all(14),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                const Icon(Icons.location_on_rounded, color: cGreen, size: 16),
                const SizedBox(width: 8),
                Text(widget.clockInRecord?.branch.name ?? 'Auckland Airport',
                    style: const TextStyle(color: cGreenText, fontWeight: FontWeight.w700, fontSize: 13)),
                const Spacer(),
                const Text('GPS verified ✓', style: TextStyle(color: cGreen, fontSize: 11))]),
          ...
          raisedCard(radius: 12, child: Padding(padding: const EdgeInsets.all(14),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Add a note (optional)',
                  style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: cText)),
              const SizedBox(height: 10),
              TextField(controller: noteCtrl, maxLines: 3,
                style: const TextStyle(fontSize: 13, color: cText),
                decoration: InputDecoration(
                  hintText: 'e.g. Completed vehicle drop-off at 3pm',
                  hintStyle: const TextStyle(color: cMuted, fontSize: 12),
                  filled: true, fillColor: cSlate,
          ...
          const SizedBox(height: 20),
          GestureDetector(onTap: () => setState(() => confirmed = true),
            child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 15),
              decoration: BoxDecoration(
      ...
      Expanded(child: _bgGradient(child: SingleChildScrollView(padding: const EdgeInsets.all(16),
        child: Column(children: [
          raisedCard(radius: 16, child: Padding(padding: const EdgeInsets.all(18),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(width: 6, height: 6, decoration: BoxDecoration(color: cBlue, borderRadius: BorderRadius.circular(3))),
                const SizedBox(width: 8),
                Text('Week 1 · 1–7 $month', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: cText))]),
              const SizedBox(height: 14),
              _SummaryRow(label: 'Mon 1 $month', value: '8.0 hrs', bold: false),
              const SizedBox(height: 7), _SummaryRow(label: 'Tue 2 $month', value: '8.5 hrs', bold: false),
              const SizedBox(height: 7), _SummaryRow(label: 'Wed 3 $month', value: '8.0 hrs', bold: false),
              const SizedBox(height: 7), _SummaryRow(label: 'Thu 4 $month', value: '7.5 hrs', bold: false),
              const SizedBox(height: 7), _SummaryRow(label: 'Fri 5 $month', value: '8.0 hrs', bold: false),
              const Divider(height: 20, color: cBorder),
          ...
          raisedCard(radius: 16, child: Padding(padding: const EdgeInsets.all(18),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(width: 6, height: 6, decoration: BoxDecoration(color: cBlue, borderRadius: BorderRadius.circular(3))),
                const SizedBox(width: 8),
                Text('Week 2 · 8–14 $month', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: cText))]),
              const SizedBox(height: 14),
          ...
          GestureDetector(onTap: () => ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 15),
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [Color(0xFF1E3A8A), cBlue]),
                borderRadius: BorderRadius.circular(12),
    ...
    return Column(children: [
      ...
      Expanded(child: _bgGradient(child: SingleChildScrollView(padding: const EdgeInsets.all(18),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 20),
          StatCard(icon: Icons.group_rounded, label: 'Total Staff', value: staff.length),
          const SizedBox(height: 10),
...
class _AdminHomeState extends State<AdminHome> {
  ...
  void _add() {
    ...
    if (!RegExp(r'^[\w.-]+@[\w.-]+\.\w+$').hasMatch(eCtrl.text.trim()))
      { setState(() => formErr = 'Valid email is required.'); return; }
    ...
    raisedCard(radius: 10, child: Padding(padding: const EdgeInsets.all(12),
      child: Row(children: [
        Container(width: 34, height: 34, decoration: BoxDecoration(
            color: iconColor.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(margin: const EdgeInsets.only(bottom: 14),
            ...
            child: DropdownButtonHideUnderline(child: DropdownButton<String>(
              ...
              items: ['Staff','Manager','Admin'].map((r) =>
          ...
          PrimaryButton(label: 'Add User', onTap: _add,
              colors: [cPurple, const Color(0xFF4F46E5)], shadowColor: const Color(0xFF3730A3))])))]);
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          GestureDetector(onTap: () => setState(() { adding = true; successMsg = ''; }),
            child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 13),
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [cPurple, Color(0xFF4F46E5)]),
                borderRadius: BorderRadius.circular(12),
          ...
          const SizedBox(height: 20),
          Text('All Users (${widget.users.length})',
              style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: cNavyMid)),
          const SizedBox(height: 8),
