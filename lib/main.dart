...
class SupabaseService {
  ...
  static Future<List<AppUser>> getUsers() async {
    final res = await _supabase.from('users').select().order('role').order('name');
  ...
  static Future<void> submitTimesheet({
    required String userEmail, required String userName,
    required String branchName, required DateTime fnStart,
    required DateTime fnEnd, required double week1,
    required double week2, required double total,
  }) async {
  ...
  static Future<void> reviewTimesheet({
    required String id, required String status, required String comment,
  }) async {
    await _supabase.from('timesheet_submissions').update({
  ...
  static Future<void> savePayrollSummary({
    required String userEmail, required String userName,
    required String branchName, required DateTime fnStart,
    required DateTime fnEnd, required double totalHours, required String savedBy,
  }) async {
    await _supabase.from('payroll_summaries').upsert({
      'user_email': userEmail, 'user_name': userName, 'branch_name': branchName,
      'fortnight_start': fnStart.toIso8601String().substring(0, 10),
      'fortnight_end': fnEnd.toIso8601String().substring(0, 10),
      'total_hours': totalHours, 'saved_at': DateTime.now().toIso8601String(),
      'saved_by': savedBy,
    });
  }
}
...
class _AppShellState extends State<_AppShell> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: switch (screen) {
...
class StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: cfg[0] as Color,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: (cfg[1] as Color).withOpacity(0.3))),
      child: Text(cfg[2] as String, style: TextStyle(color: cfg[1] as Color,
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    if (eCtrl.text.trim().isEmpty) { setState(() => err = 'Email is required.'); return; }
    if (!RegExp(r'^[\w.-]+@[\w.-]+\.\w+$').hasMatch(eCtrl.text.trim())) {
      setState(() => err = 'Invalid email format.'); return; }
    if (pCtrl.text.isEmpty) { setState(() => err = 'Password is required.'); return; }
    setState(() { loading = true; err = ''; });
    try {
      final user = await SupabaseService.login(eCtrl.text.trim(), pCtrl.text);
      if (!mounted) return;
      if (user == null) { setState(() { err = 'Incorrect email or password.'; loading = false; }); return; }
      setState(() => loading = false);
      widget.onLogin(user);
    } catch (e) {
      if (mounted) setState(() { err = 'Login error: $e'; loading = false; });
    }
  }
      ...
      child: Column(children: [
        ...
        StyledInput(hint: 'Password', ctrl: pCtrl, obscure: !showPw,
          suffix: IconButton(icon: Icon(showPw ? Icons.visibility_off : Icons.visibility,
              color: cMuted, size: 20), onPressed: () => setState(() => showPw = !showPw))),
...
class _StaffHomeState extends State<StaffHome> {
  ...
  Widget build(BuildContext context) {
    ...
    if (subScreen == 'timesheet')
      return FortnightlyTimesheetScreen(user: widget.user,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(width: double.infinity, padding: const EdgeInsets.all(18),
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: cNavy.withOpacity(0.35), blurRadius: 12, offset: const Offset(0, 4))]),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [RoleBadge(widget.user.role), const SizedBox(width: 8),
                Text(widget.user.name, style: const TextStyle(color: Colors.white,
                    fontWeight: FontWeight.w800, fontSize: 18))]),
              const SizedBox(height: 4),
              Text(widget.user.email, style: const TextStyle(color: Colors.white60, fontSize: 13))])),
          const SizedBox(height: 24),
          const Text('Administration', style: TextStyle(fontWeight: FontWeight.w700,
              fontSize: 14, color: cNavyLight)),
          const SizedBox(height: 14),
          _NavTile(icon: Icons.manage_accounts_rounded, label: 'User Management',
...
class _UserManagementScreenState extends State<_UserManagementScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() { users = u; loading = false; });
    } catch (e) {
  ...
  Future<void> _deleteUser(AppUser u) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Delete User', style: TextStyle(fontWeight: FontWeight.w800, color: cNavyMid)),
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${u.name} deleted.'), backgroundColor: cGreen));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) => Column(children: [
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('User Management', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 22)),
            Text('Add, edit and delete users', style: TextStyle(color: Colors.white70, fontSize: 13))])),
          GestureDetector(onTap: () => _openForm(),
            child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
              decoration: BoxDecoration(color: Colors.white.withOpacity(0.2),
                  borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.white30)),
              child: const Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.person_add_rounded, color: Colors.white, size: 16),
                SizedBox(width: 6),
                Text('Add User', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 13))])))]),
      ])),
    Expanded(child: _bgGradient(child:
      ...
      : users.isEmpty
          ? const Center(child: Text('No users found.', style: TextStyle(color: cMuted, fontSize: 14)))
              ...
              child: ListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  return Container(margin: const EdgeInsets.only(bottom: 10),
                    child: raisedCard(radius: 12, child: Padding(padding: const EdgeInsets.all(14),
                      child: Row(children: [
                        ...
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(u.name, style: const TextStyle(fontWeight: FontWeight.w700,
                              fontSize: 14, color: cText), overflow: TextOverflow.ellipsis),
                        ...
                        const SizedBox(width: 8),
                        Column(children: [
                          GestureDetector(onTap: () => _openForm(user: u),
                            child: Container(padding: const EdgeInsets.all(8),
                              decoration: BoxDecoration(color: cVioletLight,
                                  borderRadius: BorderRadius.circular(8)),
                              child: const Icon(Icons.edit_rounded, color: cVioletText, size: 16))),
                          const SizedBox(height: 6),
                          GestureDetector(onTap: () => _deleteUser(u),
                            child: Container(padding: const EdgeInsets.all(8),
                              decoration: BoxDecoration(color: const Color(0xFFFDECEA),
                                  borderRadius: BorderRadius.circular(8)),
...
class _UserFormScreenState extends State<_UserFormScreen> {
  ...
  Future<void> _save() async {
    if (nCtrl.text.trim().isEmpty) { setState(() => err = 'Name is required.'); return; }
    if (!RegExp(r'^[\w.-]+@[\w.-]+\.\w+$').hasMatch(eCtrl.text.trim())) {
      setState(() => err = 'Valid email is required.'); return; }
    if (pCtrl.text.length < 8) { setState(() => err = 'Password must be at least 8 characters.'); return; }
    ...
    try {
      ...
      if (isEdit) { await SupabaseService.updateUser(u); }
      else { await SupabaseService.addUser(u); }
      if (mounted) {
        widget.onSaved();
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(isEdit ? 'User updated successfully!' : 'User added successfully!'),
            backgroundColor: cGreen));
      }
    } catch (e) {
      if (mounted) setState(() { err = e.toString(); saving = false; });
    }
  }
    ...
    body: SafeArea(child: Column(children: [
      Container(padding: const EdgeInsets.fromLTRB(16, 14, 16, 18),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(isEdit ? 'Update user details' : 'Create a new user account',
              style: const TextStyle(color: Colors.white70, fontSize: 13))])),
      Expanded(child: _bgGradient(child: SingleChildScrollView(padding: const EdgeInsets.all(20),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const SizedBox(height: 8),
          raisedCard(radius: 14, child: Padding(padding: const EdgeInsets.all(20),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              StyledInput(hint: 'Min 8 characters', ctrl: pCtrl, obscure: !showPw,
                suffix: IconButton(icon: Icon(showPw ? Icons.visibility_off : Icons.visibility,
                    color: cMuted, size: 20), onPressed: () => setState(() => showPw = !showPw))),
              ...
              Container(margin: const EdgeInsets.only(bottom: 14),
                ...
                child: DropdownButtonHideUnderline(child: DropdownButton<String>(
                  value: role, isExpanded: true, padding: const EdgeInsets.symmetric(horizontal: 14),
                  items: ['Staff','Manager','Admin'].map((r) => DropdownMenuItem(value: r,
                      child: Row(children: [RoleBadge(r), const SizedBox(width: 8),
                        Text(r, style: const TextStyle(fontSize: 14))]))).toList(),
...
class _ManagerHomeState extends State<ManagerHome> {
  ...
  Widget build(BuildContext context) {
    if (subScreen == 'staff')
      return _StaffMembersScreen(users: users,
          onBack: () => setState(() => subScreen = null), onLogout: widget.onLogout);
    if (subScreen == 'timesheets')
      return _ManagerTimesheetsScreen(manager: widget.user,
          onBack: () => setState(() => subScreen = null), onLogout: widget.onLogout);
          ...
          Container(width: double.infinity, padding: const EdgeInsets.all(18),
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: cNavy.withOpacity(0.35), blurRadius: 12, offset: const Offset(0, 4)),
                const BoxShadow(color: cNavy, blurRadius: 0, offset: Offset(0, 4))]),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Dashboard', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 22)),
              Text('Auckland · Fortnight ${fmtDateShort(fnStart)} – ${fmtDate(fortnightEnd(fnStart))}',
                  style: const TextStyle(color: Colors.white60, fontSize: 12))])),
          const SizedBox(height: 20),
          const Text('Management', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: cNavyLight)),
          const SizedBox(height: 12),
          _ManagerTile(icon: Icons.people_alt_rounded, label: 'Staff Members',
              subtitle: 'View all staff profiles',
              gradient: [const Color(0xFF065F46), cGreen], shadowColor: const Color(0xFF064E3B),
              badge: '${users.where((u) => u.role == 'Staff').length}',
              onTap: () => setState(() => subScreen = 'staff')),
          const SizedBox(height: 12),
          _ManagerTile(icon: Icons.receipt_long_rounded, label: 'Time Sheets',
...
class _ManagerTile extends StatelessWidget {
  ...
  const _ManagerTile({required this.icon, required this.label, required this.subtitle,
      required this.gradient, required this.shadowColor, required this.badge, required this.onTap});
  @override
  Widget build(BuildContext context) => GestureDetector(onTap: onTap,
    child: Container(width: double.infinity, padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
      ...
      child: Row(children: [
        ...
        if (badge != null) ...[
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
            ...
            child: Text(badge!, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 14))),
          const SizedBox(width: 6)],
        const Icon(Icons.chevron_right_rounded, color: Colors.white54, size: 22)])));
}
...
class _StaffMembersScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Container(width: double.infinity, padding: const EdgeInsets.fromLTRB(16, 14, 16, 18),
        decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF065F46), cGreen],
            begin: Alignment.topLeft, end: Alignment.bottomRight)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          GestureDetector(onTap: onBack, child: const Row(children: [
            ...
            SizedBox(width: 4), Text('Back', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w600))])),
          const SizedBox(height: 8),
          const Text('Staff Members', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 22)),
          Text('${staff.length} staff registered', style: const TextStyle(color: Colors.white70, fontSize: 13))])),
      Expanded(child: _bgGradient(child:
        staff.isEmpty
          ? const Center(child: Text('No staff members yet.', style: TextStyle(color: cMuted, fontSize: 14)))
          : ListView.builder(padding: const EdgeInsets.all(16), itemCount: staff.length,
              itemBuilder: (_, i) {
                final u = staff[i];
                return Container(margin: const EdgeInsets.only(bottom: 10),
                  child: raisedCard(radius: 12, child: Padding(padding: const EdgeInsets.all(14),
                    child: Row(children: [
                      Container(width: 44, height: 44,
                        decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF065F46), cGreen]),
                            borderRadius: BorderRadius.circular(22),
                            boxShadow: [BoxShadow(color: cGreen.withOpacity(0.35), blurRadius: 6, offset: const Offset(0, 2))]),
                        child: Center(child: Text(u.name[0].toUpperCase(),
...
class _ClockInScreenState extends State<ClockInScreen> {
  ...
  Future<void> _confirmClockIn() async {
    ...
    try {
      ...
      final id = await SupabaseService.saveClockIn(user: widget.user, branch: detectedBranch!,
          lat: capturedLat!, lng: capturedLng!, clockInTime: ts, photoUrl: photoUrl);
        ...
        raisedCard(radius: 16, child: Padding(padding: const EdgeInsets.all(20),
          child: Column(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 8),
              Container(decoration: BoxDecoration(color: cSlate, borderRadius: BorderRadius.circular(10),
      ...
      child: Column(children: [
        ...
        GestureDetector(onTap: onVerify,
          child: Container(padding: const EdgeInsets.symmetric(vertical: 8),
            decoration: BoxDecoration(gradient: const LinearGradient(colors: [cNavyMid, cNavy]),
                borderRadius: BorderRadius.circular(8)),
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
  ...
  Future<void> _confirmOut() async {
    ...
    try {
      if (widget.clockInRecord?.supabaseId != null)
        await SupabaseService.saveClockOut(recordId: widget.clockInRecord!.supabaseId!,
            clockOutTime: DateTime.now(), notes: noteCtrl.text.trim(), shiftHours: _shiftHours());
        ...
        if (confirmed) ...[
          ...
          raisedCard(radius: 16, child: Padding(padding: const EdgeInsets.all(18),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _SummaryRow(label: 'Shift total', value: '${_shiftHours().toStringAsFixed(1)} hrs',
                  bold: true, valueColor: cBlue)]))),
          const SizedBox(height: 12),
...
class FortnightlyTimesheetScreen extends StatefulWidget {
  ...
  @override
  State<FortnightlyTimesheetScreen> createState() => _FortnightlyTimesheetScreenState();
}
class _FortnightlyTimesheetScreenState extends State<FortnightlyTimesheetScreen> {
  ...
  Future<void> _loadHistory() async {
    ...
    } catch (_) { setState(() => historyLoading = false); }
  }
  ...
  Future<void> _submit() async {
    ...
    final confirmed = await showDialog<bool>(context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Submit your fortnightly timesheet to your manager for review?',
              style: TextStyle(color: cNavyLight, fontSize: 13)),
          const SizedBox(height: 12),
          Container(padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(color: cSlate, borderRadius: BorderRadius.circular(8)),
            child: Column(children: [
    ...
    try {
      await SupabaseService.submitTimesheet(userEmail: widget.user.email, userName: widget.user.name,
          branchName: shiftRecords.isNotEmpty ? shiftRecords.first['branch_name'] ?? '' : '',
          fnStart: fnStart, fnEnd: fnEnd, week1: w1, week2: w2, total: total);
      if (mounted) {
        _load();
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Timesheet submitted successfully!'), backgroundColor: cGreen));
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error: $e'), backgroundColor: cRed));
  ...
  Widget _buildWeekCard(int week) {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      rows.add(Padding(padding: const EdgeInsets.only(bottom: 6),
        child: Row(children: [
          ...
          const Spacer(),
          hrs > 0
              ? Text('${hrs.toStringAsFixed(1)} hrs', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: hrs > 10 ? cOrange : cText))
              : const Text('—', style: TextStyle(fontSize: 12, color: cBorder))])));
    }
    return raisedCard(radius: 14, child: Padding(padding: const EdgeInsets.all(16),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Container(width: 6, height: 6, decoration: BoxDecoration(color: cBlue, borderRadius: BorderRadius.circular(3))),
          const SizedBox(width: 8),
          Text('Week $week · ${fmtDateShort(weekStart)} – ${fmtDateShort(weekStart.add(const Duration(days: 6)))}',
              style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: cText))]),
        const SizedBox(height: 12),
        ...rows,
        const Divider(height: 16, color: cBorder),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Week $week Total', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: cText)),
          Text('${weekTotal.toStringAsFixed(1)} hrs',
              style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13, color: weekTotal > 45 ? cOrange : cBlue))])])));
  }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('${fmtDateShort(fnStart)} – ${fmtDate(fnEnd)}', style: const TextStyle(color: Colors.white70, fontSize: 13))])),
          ...
          Row(children: [
            ...
            const SizedBox(width: 8),
            _HeaderTab(label: 'History', selected: viewingHistory, onTap: () {
      ...
      Expanded(child: _bgGradient(child:
        ...
        : viewingHistory
            ? (historyLoading ? const Center(child: CircularProgressIndicator(color: cBlue))
                : RefreshIndicator(onRefresh: _loadHistory, color: cBlue,
                    child: ListView(padding: const EdgeInsets.all(16), children: [
                      if (history.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(40),
                          child: Text('No past timesheets yet.', style: TextStyle(color: cMuted, fontSize: 14))))
                      else ...history.map(_buildHistoryCard)])))
            : RefreshIndicator(onRefresh: _load, color: cBlue,
                child: SingleChildScrollView(physics: const AlwaysScrollableScrollPhysics(),
                  padding: const EdgeInsets.all(16),
                  child: Column(children: [
                    if (submission != null) ...[_StatusBanner(submission: submission!), const SizedBox(height: 12)],
                    _buildWeekCard(1), const SizedBox(height: 12), _buildWeekCard(2),
                    const SizedBox(height: 12),
                    raisedCard(radius: 12, child: Padding(padding: const EdgeInsets.all(16),
                      child: _SummaryRow(label: 'Fortnight Grand Total',
                          value: '${total.toStringAsFixed(1)} hrs', bold: true, valueColor: cBlue))),
                    const SizedBox(height: 16),
                    if (!_isApproved)
                      GestureDetector(onTap: _submit,
                        child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 14),
                          decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF1E3A8A), cBlue]),
                              borderRadius: BorderRadius.circular(12)),
                          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                            Icon(_isSubmitted ? Icons.refresh_rounded : Icons.send_rounded, color: Colors.white, size: 18),
                            const SizedBox(width: 8),
                            Text(_isSubmitted ? 'Re-submit Timesheet' : 'Submit to Manager',
  ...
  Widget _buildHistoryCard(Map<String, dynamic> ts) {
    ...
    return Container(margin: const EdgeInsets.only(bottom: 10),
      child: raisedCard(radius: 12, child: Padding(padding: const EdgeInsets.all(14),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(start != null && end != null ? '${fmtDateShort(start)} – ${fmtDate(end)}' : '—',
                style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: cText))),
            StatusBadge(status)]),
          const SizedBox(height: 8),
          _SummaryRow(label: 'Week 1', value: '${(ts['week1_hours'] ?? 0).toStringAsFixed(1)} hrs', bold: false),
          const SizedBox(height: 3),
          _SummaryRow(label: 'Week 2', value: '${(ts['week2_hours'] ?? 0).toStringAsFixed(1)} hrs', bold: false),
          const Divider(height: 12, color: cBorder),
          _SummaryRow(label: 'Total', value: '${(ts['total_hours'] ?? 0).toStringAsFixed(1)} hrs', bold: true, valueColor: cBlue),
          if (status == 'rejected' && (ts['manager_comment'] ?? '').isNotEmpty) ...[
            ...
            Container(padding: const EdgeInsets.all(8),
              ...
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Icon(Icons.info_outline, color: cRed, size: 13), const SizedBox(width: 6),
                Expanded(child: Text('Manager: ${ts['manager_comment']}', style: const TextStyle(fontSize: 11, color: cRed)))]))]]))));
  }
}
...
class _StatusBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Color bg, fg; IconData icon; String msg;
    switch (status) {
      case 'approved':
        bg = cGreenLight; fg = cGreenText; icon = Icons.check_circle_rounded;
        msg = 'Approved${submission['approved_at'] != null ? ' on ${_fmtDT(submission['approved_at'])}' : ''}'; break;
      case 'rejected':
        bg = const Color(0xFFFDECEA); fg = cRed; icon = Icons.cancel_rounded;
        msg = 'Rejected by manager${(submission['manager_comment'] ?? '').isNotEmpty ? ': ${submission['manager_comment']}' : ''}'; break;
      case 'review':
        bg = cVioletLight; fg = cVioletText; icon = Icons.visibility_rounded;
        msg = 'Under review by manager'; break;
      default:
        bg = cAmberLight; fg = cAmberText; icon = Icons.hourglass_empty_rounded;
    ...
    return Container(padding: const EdgeInsets.all(12),
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Icon(icon, color: fg, size: 16), const SizedBox(width: 8),
        Expanded(child: Text(msg, style: TextStyle(color: fg, fontSize: 12, fontWeight: FontWeight.w600, height: 1.4)))]));
  }
}
...
class _ManagerTimesheetsScreen extends StatefulWidget {
  ...
  @override
  State<_ManagerTimesheetsScreen> createState() => _ManagerTimesheetsScreenState();
}
class _ManagerTimesheetsScreenState extends State<_ManagerTimesheetsScreen> {
  ...
  Future<void> _review(String id, String status, String comment) async {
    try {
      await SupabaseService.reviewTimesheet(id: id, status: status, comment: comment);
      _load();
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error: $e'), backgroundColor: cRed));
  ...
  Future<void> _savePayroll(Map<String, dynamic> ts) async {
    try {
      await SupabaseService.savePayrollSummary(userEmail: ts['user_email'], userName: ts['user_name'],
          branchName: ts['branch_name'] ?? '', fnStart: fnStart, fnEnd: fnEnd,
          totalHours: (ts['total_hours'] as num).toDouble(), savedBy: widget.manager.name);
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Payroll saved for ${ts['user_name']}'), backgroundColor: cGreen));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error: $e'), backgroundColor: cRed));
    }
  ...
  void _showReviewDialog(Map<String, dynamic> ts) {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(
      builder: (ctx, setDlg) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text('Review: ${ts['user_name']}',
            style: const TextStyle(fontWeight: FontWeight.w800, color: cNavyMid, fontSize: 16)),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Row(children: ['approved','rejected','review'].map((s) => Expanded(
            child: Padding(padding: const EdgeInsets.only(right: 4),
              child: GestureDetector(onTap: () => setDlg(() => selectedStatus = s),
                child: Container(padding: const EdgeInsets.symmetric(vertical: 8),
                  decoration: BoxDecoration(color: selectedStatus == s ? cNavyMid : cSlate,
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(color: selectedStatus == s ? cNavyMid : cBorder)),
                  child: Text(s[0].toUpperCase() + s.substring(1), textAlign: TextAlign.center,
                      style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                          color: selectedStatus == s ? Colors.white : cMuted))))))).toList()),
          const SizedBox(height: 12),
          TextField(controller: commentCtrl, maxLines: 3,
            decoration: InputDecoration(hintText: 'Manager comment (optional)',
              hintStyle: const TextStyle(color: cMuted, fontSize: 12), filled: true, fillColor: cSlate,
              contentPadding: const EdgeInsets.all(10),
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: cBorder)),
              enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: cBorder))))]),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          ElevatedButton(onPressed: () { Navigator.pop(ctx); _review(ts['id'], selectedStatus, commentCtrl.text.trim()); },
            style: ElevatedButton.styleFrom(backgroundColor: cNavyMid, foregroundColor: Colors.white,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
    ...
    Container(width: double.infinity, padding: const EdgeInsets.fromLTRB(16, 14, 16, 18),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        GestureDetector(onTap: widget.onBack, child: const Row(children: [
          ...
          Text('Back', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w600))])),
        const SizedBox(height: 8),
        const Text('Time Sheets', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 22)),
        Text('${fmtDateShort(fnStart)} – ${fmtDate(fnEnd)} · ${submissions.length} submitted',
            style: const TextStyle(color: Colors.white70, fontSize: 13))])),
    Expanded(child: _bgGradient(child:
      loading ? const Center(child: CircularProgressIndicator(color: cBlue))
      : err.isNotEmpty ? Center(child: Text('Error: $err', style: const TextStyle(color: cRed)))
      : RefreshIndicator(onRefresh: _load, color: cBlue,
          child: submissions.isEmpty
            ? const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.inbox_rounded, color: cMuted, size: 48), SizedBox(height: 12),
                Text('No timesheets submitted yet.', style: TextStyle(color: cMuted, fontSize: 14, fontWeight: FontWeight.w600))]))
            : ListView.builder(padding: const EdgeInsets.all(16), itemCount: submissions.length,
                itemBuilder: (_, i) {
                  ...
                  return Container(margin: const EdgeInsets.only(bottom: 12),
                    child: raisedCard(radius: 14, child: Column(children: [
                      InkWell(onTap: () => setState(() => expandedId = isExpanded ? null : ts['id']),
                        borderRadius: BorderRadius.circular(14),
                        child: Padding(padding: const EdgeInsets.all(14),
                          child: Row(children: [
                            Container(width: 44, height: 44,
                              decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF1E3A8A), cBlue]),
                                  borderRadius: BorderRadius.circular(22)),
                              child: Center(child: Text((ts['user_name'] ?? '?')[0].toUpperCase(),
                                  style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 18)))),
                            const SizedBox(width: 12),
                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Text(ts['user_name'] ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: cText), overflow: TextOverflow.ellipsis),
                              Text('${totalHrs.toStringAsFixed(1)} hrs · Submitted ${_fmtDT(ts['submitted_at'])}',
                                  style: const TextStyle(fontSize: 11, color: cMuted)),
                              const SizedBox(height: 4), StatusBadge(status)])),
                            const SizedBox(width: 8),
                            Icon(isExpanded ? Icons.expand_less : Icons.expand_more, color: cMuted, size: 20)]))),
                      if (isExpanded) Padding(padding: const EdgeInsets.fromLTRB(14, 0, 14, 14),
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Divider(height: 1, color: cBorder), const SizedBox(height: 12),
                          raisedCard(child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
                            _SummaryRow(label: 'Week 1', value: '${(ts['week1_hours'] as num?)?.toStringAsFixed(1) ?? '0.0'} hrs', bold: false),
                            const SizedBox(height: 6),
                            _SummaryRow(label: 'Week 2', value: '${(ts['week2_hours'] as num?)?.toStringAsFixed(1) ?? '0.0'} hrs', bold: false),
                            const Divider(height: 16, color: cBorder),
                            _SummaryRow(label: 'Total', value: '${totalHrs.toStringAsFixed(1)} hrs', bold: true, valueColor: cBlue)]))),
                          const SizedBox(height: 10),
                          Row(children: [
                            Expanded(child: GestureDetector(onTap: () => _showReviewDialog(ts),
                              child: Container(padding: const EdgeInsets.symmetric(vertical: 10),
                                decoration: BoxDecoration(gradient: const LinearGradient(colors: [cNavyMid, cNavy]),
                                    borderRadius: BorderRadius.circular(8)),
                                child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                                  Icon(Icons.rate_review_rounded, color: Colors.white, size: 14), SizedBox(width: 6),
                                  Text('Review', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 12))])))),
                            const SizedBox(width: 8),
                            Expanded(child: GestureDetector(onTap: () => _savePayroll(ts),
                              child: Container(padding: const EdgeInsets.symmetric(vertical: 10),
                                decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF065F46), cGreen]),
                                    borderRadius: BorderRadius.circular(8)),
                                child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
